...
for key,value in game_name.items():
  ...
  #print("counting"+str(int(key)+"/"+str(len(game.items)))
  conneted_sqlite.execute("INSERT INTO npsave(game) VALUES (?);",(key,))
  ...
  #conneted_sqlite.execute('update ttes set positive="{}", negative="{}"where\
      #game="{}"'.format(value[1],value[2],key))
  conn.commit()
d.execute("delete from npsave where id not in(select min(id) from npsave group by game)")
conn.commit()
